...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase().toList.groupBy((c: Char) => c).toList.sortBy(_._1).map { case (c, l) => (c, l.length) }
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def concateSentence(s: Sentence): Word =
      if (s.isEmpty) ""
      else
        s.head + concateSentence(s.tail)

    wordOccurrences(concateSentence(s))
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    def subtractPair(pair: List[(Char, Int)]): (Char, Int) = pair match {
      case List(x) => x
      case List((ch, i), (_, j)) => (ch, i - j)
    ...
    if (x.isEmpty) Nil
      ...
      (x ++ y).groupBy(_._1).mapValues(subtractPair).values.filterNot(e => e._2 == 0).toList.sortBy(_._1)
    }
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def sentenceAnagrams0(occurrences: Occurrences): List[Sentence] = occurrences match {
      ...
      case occs => for {
        ...
        sentence <- sentenceAnagrams0(subtract(occs,wordOccurrences(word)))
